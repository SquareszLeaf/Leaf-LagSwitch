...
def block_all_apps():
    run_netsh_command('netsh advfirewall firewall add rule name="Block_All" dir=out action=block')

def unblock_all_apps():
    run_netsh_command('netsh advfirewall firewall delete rule name="Block_All"')

def block_apps(apps):
    for app in apps:
        run_netsh_command(f'netsh advfirewall firewall add rule name="Block_{app}" dir=out action=block program="{app}"')

def unblock_apps(apps):
    for app in apps:
        run_netsh_command(f'netsh advfirewall firewall delete rule name="Block_{app}"')
